...
public class MemberDAO {
	...
	public String findQuest(String id) throws Exception{
		...
		try {
			...
			//sql문 작성
			sql = "SELECT * FROM hmember JOIN hmember_detail USING(mem_num) "
					+ "JOIN hmember_qna USING(mem_pwq) WHERE mem_id=?";
	...
	//비밀번호 변경(비밀번호 모를 시!!)
	public String changePassword(String newPw, String mem_id) throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = null;
		...
		try {
			...
			MemberVO vo = checkMember(mem_id);
			
			//SQL문 작성
			sql = "UPDATE hmember_detail SET mem_pw=? WHERE mem_num=?";
			
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			
			//?에 데이터를 바인딩
			pstmt.setString(1, newPw);//새비밀번호
			pstmt.setInt(2, vo.getMem_num());//회원번호
	...
	public int getMemberCount()throws Exception {
		...
		try {
			...
			conn = DBUtil.getConnection();
			//SQL문 작성
			sql = "SELECT COUNT(*) FROM hmember_detail";
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			//SQL문 실행
			rs = pstmt.executeQuery();
