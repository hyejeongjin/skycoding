		...
		<div class="content-main"><!-- 전체 화면의 80% -->
			...
			<div class="content-right">
				<div>
					...
					<h2>${freeBoard.free_title}</h2>
					<%--로그인한 회원번호와 작성자 회원번호가 일치해야 수정,삭제 가능 --%>
					<div class="top-button">
					<c:if test="${mem_num == freeBoard.mem_num}">
						<input type="button" value="수정" class="btn btn-primary btn-sm" 
								onclick="location.href='updateForm.do?free_id=${freeBoard.free_id}'">
					</c:if>
					<%--관리자는 본인 글 아니어도 삭제 가능 --%>
					<c:if test="${mem_num == freeBoard.mem_num || mem_auth == 9}">
					...
					<script type="text/javascript">
						$(function(){
							$('#delete-btn').click(function(){
								...
								if(choice){
									location.replace('delete.do?free_id=${freeBoard.free_id}');
								}
							});
						});
					</script>
					...
					<ul class="detail-info2">
						...
						<c:if test="${!empty freeBoard.free_modify_date}">
						<li>수정일  ${freeBoard.free_modify_date}
						</c:if>
						<li><i class="fa-solid fa-eye"></i> ${freeBoard.free_hit}</li>
					</ul>
					</div>
					<hr class="hr-style" size="1" width="100%">
					<%--이미지가 있을 때만 보여지게 --%>
					<c:if test="${!empty freeBoard.free_photo}">
						<img src="${pageContext.request.contextPath}/upload/${freeBoard.free_photo}" class="detail-img">
					</c:if>	
					...
					<div id="reply_div">
						...
						<form id="re_form">
							<!-- submit하면 ajax 통신하도록 만듦. freeBoard.reply.js에서 댓글 등록 부분 참고 -->
					...
					<div class="col-sm-12 col-md-6">
						<ul class="bottom-prevNext">
   							<li>
  								...
  								다음글 | <a href="detail.do?free_id=${pnBoard.next}">${pnBoard.next_title}</a>
  								</c:if>
							</li>
							<li>
								<c:if test="${pnBoard.prev!=0}">
   								이전글 | <a href="detail.do?free_id=${pnBoard.prev}">${pnBoard.prev_title}</a>
   								</c:if>
   							</li>
						</ul>
					</div>
