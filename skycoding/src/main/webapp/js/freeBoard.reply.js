$(function(){
	...
	function selectList(pageNum){
		...
		$.ajax({
			url:'listReply.do',
			type:'post',
			data:{pageNum:pageNum,free_id:$('#free_id').val()}, //ajax로 읽어올 정보(pageNum값, board_num값)
			dataType:'json',
			success:function(param){
				...
				$(param.list).each(function(index,item){
					...
					output += '<p>' + item.freeComm_content + '</p>';
					
					if(item.freeComm_modify_date){ //수정한 경우
						output += '<span class="modify-date">최근 수정일 : ' + item.freeComm_modify_date + '</span>';
					}else{ //수정 안 한 경우
						output += '<span class="reg-date">' + item.freeComm_reg_date + '</span>';
					}
					...
					if(param.user_num == item.mem_num){
						...
						output += '<button type="button" class="modify-btn btn btn-primary" data-renum="'+item.freeComm_id+'">수정</button>';
						output += ' <button type="button" class="delete-btn btn btn-primary" data-renum="'+item.freeComm_id+'">삭제</button>';
					}
					output += '</div>';
	...
	$('#re_form').submit(function(event){
		...
		if($('#freeComm_content').val().trim()==''){
			alert('내용을 입력하세요!');
			$('#freeComm_content').val('').focus();
			return false;
		}
	...
	$(document).on('keyup','textarea',function(){ //미래의 태그는 $(document).on(이벤트,태그,이벤트내용순) 명시
		...
		if(inputLength > 50){ //50를 넘어선 경우
			...
			remain += '/50';
			if($(this).attr('id') == 'freeComm_content'){
				//등록폼 글자수
	...
	$(document).on('click','.modify-btn',function(){
		//댓글 번호
		let freeComm_id = $(this).attr('data-renum');
		//댓글 내용		this는 수정버튼						//g:지정문자열 모두, i:대소문자 구별없이 검색
		let freeComm_content = $(this).parents('.sub-item').find('p').html().replace(/<br>/gi,'\n');
		
		//댓글 수정폼 UI(동적으로 만듦) //아래에서 .item에 붙일 건데 이게 있어야 할 자리에 수정폼을 보여지게 할 거라서 이건 숨김처리 할 거다
		let modifyUI = '<form id="mre_form">';
			modifyUI += '<input type="hidden" name="freeComm_id" id="freeComm_id" value="'+freeComm_id+'">';
			modifyUI += '<div id="mre_first"><span class="letter-count">50/50</span></div>';
			
			modifyUI += '<div class="inner-text">';
			modifyUI += '<textarea class="form-control rep-content inner-text" name="freeComm_content" id="mre_content">'+freeComm_content+'</textarea>';
			modifyUI += '</div>';
